# Chess Game

A fully functional chess game built with vanilla JavaScript, HTML, and CSS. Play chess in your browser with a clean, modern interface.

## Features

- ✅ Complete chess rules implementation
- ✅ Valid move highlighting
- ✅ Check and checkmate detection
- ✅ Captured pieces tracking
- ✅ Turn-based gameplay
- ✅ Responsive design
- ✅ Clean, modern UI

## How to Play

1. Open `index.html` in your web browser
2. Click on a piece to select it (only your pieces can be selected)
3. Valid moves will be highlighted in yellow
4. Click on a highlighted square to move your piece
5. The game alternates between white and black players
6. Click "New Game" to reset the board

## Game Rules

The game follows standard chess rules:
- **Pawns**: Move forward one square (two squares from starting position), capture diagonally
- **Rooks**: Move horizontally or vertically any number of squares
- **Knights**: Move in an "L" shape (2 squares in one direction, 1 in perpendicular)
- **Bishops**: Move diagonally any number of squares
- **Queen**: Combines rook and bishop movement
- **King**: Move one square in any direction

The game detects check and checkmate conditions automatically.

## Installation

### Option 1: Direct Download
1. Download all files from this repository
2. Open `index.html` in any modern web browser

### Option 2: Clone Repository
```bash
git clone https://github.com/yourusername/chess-game.git
cd chess-game
```

Then open `index.html` in your browser.

## Files

- `index.html` - Main HTML structure
- `style.css` - Styling and layout
- `chess.js` - Game logic and chess rules
- `README.md` - This file

## Technologies Used

- HTML5
- CSS3
- JavaScript (ES6+)

## Browser Compatibility

Works on all modern browsers:
- Chrome
- Firefox
- Safari
- Edge

## Future Enhancements

Potential features for future versions:
- En passant capture
- Castling
- Pawn promotion
- Move history
- Timer/clock
- AI opponent
- Online multiplayer

## Contributing

Feel free to fork this project and submit pull requests with improvements!

## License

This project is open source and available under the MIT License.

## Author

Created with ❤️ for chess enthusiasts

---

Enjoy your game of chess! ♟️
